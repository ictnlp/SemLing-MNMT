...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False, linguistic_encoder=False):
        super().__init__()
        self.cfg = cfg
        ...
        if linguistic_encoder:
            self.embed_dim = cfg.linguistic_encoder.embed_dim
            self.normalize_before = cfg.linguistic_encoder.normalize_before
            self.num_heads = cfg.linguistic_encoder.attention_heads
            
            self.fc1 = self.build_fc1(
                self.embed_dim,
                cfg.linguistic_encoder.ffn_embed_dim,
                self.quant_noise,
        ...
        self.self_attn = self.build_self_attention(self.embed_dim, cfg, linguistic_encoder)
    ...
    def build_self_attention(self, embed_dim, cfg, linguistic_encoder=False):
        if linguistic_encoder:
            return MultiheadAttention(
                embed_dim,
                cfg.linguistic_encoder.attention_heads,
                dropout=cfg.attention_dropout,
                self_attention=True,
                q_noise=self.quant_noise,
                qn_block_size=self.quant_noise_block_size,
                xformers_att_config=cfg.linguistic_encoder.xformers_att_config,
            )
        else:
            return MultiheadAttention(
                embed_dim,
...
class TransformerEncoderLayer(TransformerEncoderLayerBase):
    ...
    def build_self_attention(self, embed_dim, args, linguistic_encoder=False):
