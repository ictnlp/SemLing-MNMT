...
@dataclass
class LinguisticEncoderConfig(EncDecBaseConfig):
    layers: int = field(default=2, metadata={"help": "number of linguistic encoder layers"})
...
class TransformerConfig(FairseqDataclass):
    ...
    def from_namespace(cls, args):
        ...
        if not isinstance(args, cls):
            ...
            # for now.
            # fields(cls): 按定义顺序将类中字段打包成field格式
            for fld in fields(cls):
                ...
                elif fld.name == "linguistic_encoder":
                    # same but for encoder
                    if safe_hasattr(args, "linguistic_encoder"):
                        seen.add("linguistic_encoder")
                        config.linguistic_encoder = LinguisticEncoderConfig(**args.linguistic_encoder)
                    else:
                        config.linguistic_encoder = cls._copy_keys(
                            args, LinguisticEncoderConfig, "linguistic_encoder", seen
                        )
